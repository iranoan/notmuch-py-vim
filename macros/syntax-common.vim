...
" Usenet headers
syntax match	mailHeaderKey	contained	contains=mailHeaderEmail,mailEmail,@NoSpell /^\c[a-z-]\+:\s*/
...
syntax match	mailURL	contains=@NoSpell '\(\<\(\(\(https\=\|ftp\|gopher\)://\|\(mailto\|file\|news\):\)[^'' \t<>"]\+\|\(www\|web\|w3\)[A-Za-z0-9_-]*\.[A-Za-z0-9._-]\+\.[!#-&*-;=\=-Z\\^-z\|~]\+\)[A-Za-z0-9/~]\)'

syntax match	mailEmail	contains=@NoSpell '\c[_=a-z\./\+0-9-]\+@[a-z0-9._-]\+\a\{2}'

" Make sure quote markers in regions \(header / signature\) have correct color
syntax match	mailQuoteExp1	contained '^\s*\(> \=\)'
syntax match	mailQuoteExp2	contained '^\s*\(> \=\)\{2}'
syntax match	mailQuoteExp3	contained '^\s*\(> \=\)\{3}'
syntax match	mailQuoteExp4	contained '^\s*\(> \=\)\{4}'
syntax match	mailQuoteExp5	contained '^\s*\(> \=\)\{5}'
syntax match	mailQuoteExp6	contained '^\s*\(> \=\)\{6}'

" Even and odd quoted lines. Order is important here!
syntax region	mailQuoted6	keepend	contains=mailVerbatim,mailHeader,mailHeader2,@mailLinks,mailSignature,@NoSpell start='\c^\z([ \t]*\%(\%([a-z]\+>\|[]>]\)[ \t]*\)\{5}\%([ \t]*[a-z]\+>\|[ \t]*[]>]\)\)' end='^\z1\@!' fold
syntax region	mailQuoted5	keepend	contains=mailQuoted6,mailVerbatim,mailHeader,mailHeader2,@mailLinks,mailSignature,@NoSpell start='\c^\z([ \t]*\%(\%([a-z]\+>\|[]>]\)[ \t]*\)\{4}\%([ \t]*[a-z]\+>\|[ \t]*[]>]\)\)' end='^\z1\@!' fold
syntax region	mailQuoted4	keepend	contains=mailQuoted5,mailQuoted6,mailVerbatim,mailHeader,mailHeader2,@mailLinks,mailSignature,@NoSpell start='\c^\z([ \t]*\%(\%([a-z]\+>\|[]>]\)[ \t]*\)\{3}\%([ \t]*[a-z]\+>\|[ \t]*[]>]\)\)' end='^\z1\@!' fold
syntax region	mailQuoted3	keepend	contains=mailQuoted4,mailQuoted5,mailQuoted6,mailVerbatim,mailHeader,mailHeader2,@mailLinks,mailSignature,@NoSpell start='\c^\z([ \t]*\%(\%([a-z]\+>\|[]>]\)[ \t]*\)\{2}\%([ \t]*[a-z]\+>\|[ \t]*[]>]\)\)' end='^\z1\@!' fold
syntax region	mailQuoted2	keepend	contains=mailQuoted3,mailQuoted4,mailQuoted5,mailQuoted6,mailVerbatim,mailHeader,mailHeader2,@mailLinks,mailSignature,@NoSpell start='\c^\z([ \t]*\%(\%([a-z]\+>\|[]>]\)[ \t]*\)\{1}\%([ \t]*[a-z]\+>\|[ \t]*[]>]\)\)' end='^\z1\@!' fold
syntax region	mailQuoted1	keepend	contains=mailQuoted2,mailQuoted3,mailQuoted4,mailQuoted5,mailQuoted6,mailVerbatim,mailHeader,mailHeader2,@mailLinks,mailSignature,@NoSpell start='\c^\z([ \t]*[a-z]\+>\|[ \t]*[]>]\)' end='^\z1\@!' fold
